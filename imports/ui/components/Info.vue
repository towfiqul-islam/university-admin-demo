    ...
    <ul>
      <li>
        <form class="info-link-add">
          ...
          <input class="px-2 py-2 bg-red-200" type="submit" name="submit" @click="submit($event)" value="Add new link">
        </form>
      </li>
      <li v-bind:key="link" v-for="link in links"><a :href="link.url" target="_blank">{{link.title}}</a></li>
    </ul>
  </div>
</template>
